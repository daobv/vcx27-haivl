...
class FileController extends AdminController {
    ...
    public function actionUpload() {
        ...
        // Settings
        $targetDir = Yii::getPathOfAlias('webroot') . DIRECTORY_SEPARATOR . 'files' . DIRECTORY_SEPARATOR . date('Ymd');
        ...
        if (!$chunks || $chunk == $chunks - 1) {
            ...
            $f->file = '/files/' . date('Ymd') . '/' . $info['basename'];
            $f->fileId = 3;
            $f->status = 1;
            $f->save();
    ...
    private function _selectList() {
        $listData = array(File::QUOTE => 'Báo giá',File::DS=>'Tài liệu phần mền');
        return $listData;
    }
    ...
    public function actionDelete($id) {
        ...
        if (!empty($model->file))
            unlink(Yii::app()->basePath . '/../' . $model->file);
