...
class GalleryController extends AdminController {
    ...
    public function actionBatchUpload() {
        if (isset($_POST['albumId'])) {
            $category = Album::model()->findByPk($_POST['albumId']);
            $this->render('multiple_upload', array('category' => $category));
        } else {
            $this->render('choose_category');
        }
    }
    public function actionSliderUpload() {
            $this->render('slider_upload');
    }
    ...
    public function actionUpload() {
        ...
        if (!$chunks || $chunk == $chunks - 1) {
            ...
            $gallery->image = '/upload/' . date('Y-m-d') . '/' . $info['basename'];
            $gallery->albumId = $_REQUEST['albumId'];
            $gallery->dateCreated = date('Y-m-d');
            $gallery->status = 1;
            $gallery->save();
